...
import { readJson as storeRead, cached as storeCached, updateJson as storeUpdate } from './store.js';
const STORE = { readJson: storeRead, updateJson: storeUpdate };
import { providerTitle, sosEntity } from './providers.js';
import { sentinelFrom, watchRow, watchFlag, WATCH_PATH, EMPTY_WATCH } from './watch.js';
...
async function postJson(url, params, ms = REQ_MS) {
  ...
  try {
    const r = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'ojai-atlas/1.0 (+https://eco-village-map.vercel.app)' }, body: form(params), signal: ctl.signal });
...
async function getJson(url, ms = REQ_MS) {
  ...
  const t = setTimeout(() => ctl.abort(), ms);
  try {
    const r = await fetch(url, { signal: ctl.signal, headers: { 'User-Agent': 'ojai-atlas/1.0 (+https://eco-village-map.vercel.app)' } });
    if (!r.ok) throw new Error('HTTP ' + r.status);
    return await r.json();
  } finally { clearTimeout(t); }
}
...
// the watch log for one parcel (data/watch.json — the bundled copy or the newer one in the repository)
export async function loadWatch(apn10) {
  const p = String(apn10 || '').replace(/[^0-9]/g, '');
  if (!p) return null;
  let local = null, remote = null;
  try { local = JSON.parse(await readFile(new URL('../' + WATCH_PATH, import.meta.url), 'utf8')); } catch { local = null; }
  try { remote = await storeRead(WATCH_PATH, null, 300000); } catch { remote = null; }
  const pick = [local, remote].filter((w) => w && w.parcels).sort((a, b) => String(b.updatedAt || '').localeCompare(String(a.updatedAt || '')))[0] || EMPTY_WATCH;
  return (pick.parcels || {})[p] || null;
}
...
// a promise that never outstays its welcome: the ownership reads must not hold the record hostage
const within = (p, ms, dflt = null) => Promise.race([Promise.resolve(p).catch(() => dflt), new Promise((ok) => setTimeout(() => ok(dflt), ms))]);
export function composeTitle(rollRows, ev, roll, prov) {
  ...
  if (ev && ev.owner && ev.owner.names && ev.owner.names.length) {
    ...
    // no evidence on file: a provider that names the owner leads, the roll's verdict corroborates
    const named = prov && prov.rows && prov.rows.find((r) => r[2] === 'owner_provider' && !/^No /.test(String(r[1])));
    if (named) { prov.rows.forEach((r) => out.push(r)); if (rollOwnerNow) out.push([rollOwnerNow[0] + ' (public roll)', rollOwnerNow[1], 'owner_roll']); prov = null; }
...
export async function resolveCore(q) {
  ...
  // The ownership work touches no county layer — the evidence on file, the watch log and the paid
  // provider. It runs BESIDE the county fan-out, never queued inside it, so a slow county day still
  // yields a named owner, the dated title rows and the watch line. Each read has its own ceiling.
  let evidence = null, provider = null, watch = null, providerSkipped = null;
  const side = [], sideNames = [];
  if (rec.apn10) { sideNames.push('evidence'); side.push(within(loadEvidence(rec.apn10), 3000).then((v) => { evidence = v; })); }
  if (rec.apn10) { sideNames.push('watch'); side.push(within(loadWatch(rec.apn10), 3000).then((v) => { watch = v; })); }
  // A paid lookup only where it adds something: q.provider === false turns it off (the warm job
  // uses it), and a report on file that is under a year old already names the owner better than
  // a vendor's roll copy does, so the request is not spent. An older report is exactly when the
  // live copy is worth having, so it runs then.
  if (rec.apn10 && county && q.provider !== false) {
    sideNames.push('provider');
    side.push((async () => {
      const ev = await within(loadEvidence(rec.apn10), 3000);
      if (ev) evidence = ev;
      const when = ev && ((ev.source || {}).preparedOn || String(ev.importedAt || '').slice(0, 10));
      const months = when ? Math.round((Date.now() - new Date(when + 'T00:00:00Z')) / (30.44 * 86400000)) : null;
      const freshFor = (() => { const n = Number(process.env.PROVIDER_SKIP_MONTHS); return isFinite(n) && n >= 0 ? n : 12; })();
      if (ev && ev.owner && (ev.owner.names || []).length && months != null && months < freshFor) { providerSkipped = 'a report of ' + isoLong(when) + ' is on file and still fresh'; return; }
      provider = await within(providerTitle({ apn: rec.apnPretty, apn10: rec.apn10, fips: county.fips, state: county.state, lat: site.cy, lon: site.cx, situs: rec.situs, store: STORE }), 7000);
    })());
  }
  let records = [];
  ...
  // the county budget is spent; the ownership reads get their own short window (the function has 30 s)
  await within(Promise.all(side), 3500);
  ...
  if (process.env.SOS_API_KEY && evidence && evidence.owner && evidence.owner.company && evidence.owner.names && evidence.owner.names.length && Date.now() - t0 < BUDGET_MS + 2500) {
  ...
  const wrow = watchRow(watch);
  if (wrow) { const i = buckets.title.findIndex((r) => r[2] === 'title_source'); if (i >= 0) buckets.title.splice(i, 0, wrow); else buckets.title.push(wrow); }
  ...
  const diag = q.debug ? done.map((r, i) => [names[i], r.ok ? 'ok' : (r.late ? 'late' : 'error: ' + r.error), r.ms || null])
    .concat(sideNames.map((nm) => [nm + ' (beside the fan-out)', nm === 'evidence' ? (evidence ? 'ok' : 'none on file') : nm === 'watch' ? (watch ? 'ok' : 'not watched') : (provider ? 'ok' : providerSkipped ? 'skipped — ' + providerSkipped : 'no key or no match'), null])) : undefined;
  ...
  if (!(evidence && evidence.owner && evidence.owner.names && evidence.owner.names.length) && provider && provider.rows) {
    const named = provider.rows.find((r) => r[2] === 'owner_provider' && !/^No /.test(String(r[1])));
    if (named) flags.push({ level: 'good', key: 'owner_named', text: 'Owner of record per ' + String(named[0]).replace(/^Owner of record \(/, '').replace(/\)$/, '') + ': ' + String(named[1]).split(' · ')[0] + ' — a live copy of the county roll; the deed itself is at the Recorder.' });
  }
  const movedFlag = watchFlag(watch, evidence); if (movedFlag) flags.push(movedFlag);
  ...
  const out = {
    ...
    watch: watch ? { firstSeen: watch.firstSeen || null, lastChecked: watch.lastChecked || null, checks: watch.checks || 0, changes: (watch.history || []).length, latest: (watch.history || [])[0] || null } : null,
    sourcesQueried: tasks.length + side.length, sourcesAnswered: answered + side.length, sourcesLate: late, partial: late > 0,
...
export async function resolveParcel(q) {
  ...
  return { apn: rec.apnPretty, apn10: rec.apn10, situs: rec.situs, acreage: rec.acreage != null ? rec.acreage : (acresGis != null ? Number(acresGis.toFixed(2)) : null),
    ...
    county: county ? Object.assign({}, county, { adapter: adapter ? adapter.id : null }) : null,
    sentinel: adapter && parcel ? sentinelFrom(adapter.id, parcel.attributes) : null, ms: Date.now() - t0 };
}
