...
// ---- the durable miss list -------------------------------------------------------
//   RC_CACHE below lives in one serverless instance's memory, and instances recycle
//   constantly, so a parcel the vendor simply does not hold would be re-queried on every
//   cold start — spending the month's budget on the parcels least likely to ever answer.
//   The misses are therefore remembered in the repository: APNs ONLY, no names and no
//   addresses, so this is safe in a public repository. An entry expires after 60 days, so
//   a parcel the vendor adds later is eventually picked up again.
export const MISS_PATH = 'data/provider-misses.json';
const EMPTY_MISSES = { schema: 1, note: 'parcels a provider holds no record for — APNs only, no names, no addresses', rentcast: {} };
const MISS_TTL_DAYS = 60;
const today = () => new Date().toISOString().slice(0, 10);
const daysBetween = (a, b) => Math.round((new Date(b + 'T00:00:00Z') - new Date(a + 'T00:00:00Z')) / 86400000);
let missMem = null;
export async function missRead(store) {
  if (store) { try { const j = await store.readJson(MISS_PATH, null, 120000); if (j && j.rentcast) { missMem = j; return j; } } catch (e) { /* fall through */ } }
  return missMem || (missMem = JSON.parse(JSON.stringify(EMPTY_MISSES)));
}
export function missHas(misses, who, apn10, now = today()) {
  const seen = ((misses || {})[who] || {})[String(apn10 || '')];
  return !!(seen && daysBetween(seen, now) < MISS_TTL_DAYS);
}
export async function missAdd(store, who, apn10, now = today()) {
  const key = String(apn10 || '');
  if (!key) return missMem;
  const add = (cur) => {
    const base = cur && cur[who] ? cur : EMPTY_MISSES;
    const next = JSON.parse(JSON.stringify(base));
    next[who] = Object.assign({}, next[who]);
    next[who][key] = now;
    for (const k of Object.keys(next[who])) if (daysBetween(next[who][k], now) >= MISS_TTL_DAYS) delete next[who][k];
    next.updatedAt = new Date().toISOString();
    return next;
  };
  missMem = add(missMem);
  if (!store) return missMem;
  try { const r = await store.updateJson(MISS_PATH, add, 'provider: ' + who + ' holds no record for ' + key); missMem = r.data || missMem; } catch (e) { /* memory still holds it */ }
  return missMem;
}
export const missReset = () => { missMem = null; };
...
export async function rentcastTitle({ apn, apn10, lat, lon, situs, store }) {
  ...
  // a parcel the vendor has already told us it does not hold costs nothing to ask about again
  if (digits && missHas(await missRead(store), 'rentcast', digits)) {
    return { provider: 'RentCast', asOf: today(), matched: false, remembered: true,
      rows: [['Owner of record (RentCast)', 'RentCast held no record for this parcel when it was last asked, so the lookup was not spent again — the list of parcels it does not cover is remembered for 60 days. The public roll and any report on file stand.', 'owner_provider']] };
  }
  ...
  // The radius only sets how wide the net is; the match below is what keeps the answer correct.
  // 0.08 mi (130 m) was sized for a suburban lot and missed rural parcels entirely — on ten or a
  // hundred acres the vendor's point for the dwelling sits far from our computed centre. 0.6 mi
  // (~1 km) covers any parcel we are likely to look at, and a neighbour still cannot be returned.
  const q = new URLSearchParams({ latitude: String(lat), longitude: String(lon), radius: '0.6', limit: '50' });
  ...
  const rows = [];
  if (!p) {
    rows.push(['Owner of record (RentCast)', (list.length
      ? 'None of the ' + list.length + ' RentCast record' + (list.length === 1 ? '' : 's') + ' within a kilometre of this parcel is this parcel — matched on assessor id' + (situs ? ' and situs address' : '') + ', never on proximity, so a neighbour is never reported as the owner'
      : 'RentCast holds no property record within a kilometre of this parcel — the vendor’s database is strongest on suburban residential property and thins out on rural and unimproved land')
      + '. The public roll and any report on file stand.', 'owner_provider']);
    const out = { provider: 'RentCast', asOf, rows, matched: false };
    RC_CACHE.set(ck, { ts: Date.now(), data: out });
    if (digits) await missAdd(store, 'rentcast', digits);
    return out;
  }
  const o = p.owner || {};
