...
if git diff --quiet HEAD^ HEAD -- . ':(exclude)data/research.json' ':(exclude)data/uploads.json' ':(exclude)data/zone-positions.json' ':(exclude)data/watch.json' ':(exclude)data/provider-meter.json' ':(exclude)data/provider-misses.json' ':(exclude)images/uploads' ':(exclude)docs/uploads'; then
